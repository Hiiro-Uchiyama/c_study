...
double velBounceCir[2] = { 0.13, 0.14 }; //移動量のための変数
...
void update(int value) {
    ...
    if (score <= 0){
        ...
        posBounceCir[0] += velBounceCir[0];
        posBounceCir[1] += velBounceCir[1];
        if (posBounceCir[0] == 1.0 && JudgmentA == 0){
            posBounceCir[0] = -1.0;
            posBounceCir[0] -= velBounceCir[0];
            score++;
            velBounceCir[0] *= -1;
        }
        if (posBounceCir[0] == 0.95 && JudgmentA == 1){
            posBounceCir[0] = -1.0;
            posBounceCir[0] += velBounceCir[0];
            JudgmentA = 0;
            velBounceCir[0] *= -1;
        }
        if (posBounceCir[1] == 1.0 && JudgmentB == 0){
            posBounceCir[1] = -1.0;
            posBounceCir[1] -= velBounceCir[1];
            JudgmentB = 1;
            velBounceCir[0] *= -1;
        }
        if (posBounceCir[1] == 1.0 && JudgmentB == 1){
            posBounceCir[1] = -1.0;
            posBounceCir[0] -= velBounceCir[0];
            JudgmentB = 0;
            score--;
    ...
    isCrushingFrame = 0;
    if (pow(posPlayer[0], 2.0) > 0.95 || pow(posPlayer[1], 2.0) > 0.95) {
                isCrushingFrame++;
                score--;
    }
...
void display(void) {
    ...
    drawCircle(posCir, 0.03, 0, 360); //円を描画
    glColor3d(colCir[0], colCir[1], colCir[2]); //描画色の指定
