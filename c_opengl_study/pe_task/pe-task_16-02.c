...
void drawTri (struct rectangle *r){
    glColor3d(r->color[3], r->color[1], r->color[2]);
    glBegin(GL_POLYGON);
    glVertex2d(r->pos[0], r->pos[1]);
    glVertex2d(r->pos[0] + r->size[0], r->pos[1]);
    glVertex2d(r->static_pos[0] + r->size[0], r->static_pos[0] - r->size[1]);
    glEnd();
    if(r->next != NULL){
        drawTri(r->next);
    }
...
void mouse (int button, int state, int x, int y){
    ...
    for (i = 0; i < 2; i++){
        ...
        newRect->static_pos[i] = ((double)rand()/RAND_MAX)*2-1;
        if (i == 1){
            newRect->size[i] *= -1;
        }
    }
