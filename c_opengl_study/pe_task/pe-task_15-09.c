...
void Update(int value) {
    ...
    if (score < 0){
        ...
        if (Upwards == 0){
            ...
            if (m == 0){
                for (i = 0; i < 10; i++){
                    if(posDot[1] >= 0.8 && ((fabs(squere[i].Left - posDot[0])) <= 0.002 || fabs(squere[i].Right - posDot[0]) <= 0.002) && (squere[i].Low - posDot[1]) <= 0.002){
                        velDot[1] *= -1;
...
void Display(void) {
    ...
    unsigned int n;
    if (Start == 0){
        sprintf(startstr, "Let's start Block breaking");
